...
import com.ctre.phoenixpro.ecus.CANCoder;
...
public class CanCoderFactoryBuilder {
    ...
    public AbsoluteEncoderFactory<CanCoderAbsoluteConfiguration> build() {
        return configuration -> {
            ...
            config.MagnetSensor.AbsoluteSensorRange = config.MagnetSensor.AbsoluteSensorRange.Unsigned_0To360;
            config.MagnetSensor.MagnetOffset = Math.toDegrees(configuration.getOffset());
            config.MagnetSensor.SensorDirection = config.MagnetSensor.SensorDirection.Clockwise_Positive;
    ...
    private static class EncoderImplementation implements AbsoluteEncoder {
        ...
        public double getAbsoluteAngle() {
            ...
            double angle = Math.toRadians(encoder.getAbsPosition().getValue());
            angle %= 2.0 * Math.PI;
            if (angle < 0.0) {
                angle += 2.0 * Math.PI;
            }
